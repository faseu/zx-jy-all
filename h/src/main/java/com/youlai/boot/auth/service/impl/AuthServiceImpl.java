...
public class AuthServiceImpl implements AuthService {
    ...
    private final TOperLogService operLogService;
     ...
     * @param username  用户名
     * @param password  密码
     * @param captchaKey 验证码KEY（前端从获取验证码接口返回）
     * @param captchaCode 用户输入的验证码
     * @return 访问令牌
     */
    @Override
    public AuthenticationToken login(String username, String password, String captchaKey, String captchaCode) {
        // ====================== 1. 图形验证码校验 ======================
        // 从Redis获取正确的验证码
        String redisCaptchaKey = StrUtil.format(RedisConstants.Captcha.IMAGE_CODE, captchaKey);
        String correctCode = (String) redisTemplate.opsForValue().get(redisCaptchaKey);
        ...
        updateLoginTime(username, loginTime);
        saveLoginLog(username, loginTime);
     ...
     * 更新用户最后登录时间
     */
    private void updateLoginTime(String username, Date loginTime) {
        LambdaUpdateWrapper<User> wrapper = Wrappers.lambdaUpdate();
        wrapper.eq(User::getUsername, username);
        wrapper.set(User::getLoginTime, loginTime);
        userMapper.update(null, wrapper);
    }

    private void saveLoginLog(String username, Date loginTime) {
        TOperLog log = new TOperLog();
        log.setCreateBy(username);
        log.setLoginTime(loginTime);
        log.setOperateTime(loginTime);
