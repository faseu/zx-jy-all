...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        if (!"local".equalsIgnoreCase(ossType) || localStoragePath == null || localStoragePath.isBlank()) {
            return;
        }

        String resourceLocation = Paths.get(localStoragePath).toAbsolutePath().normalize().toUri().toString();
        if (!resourceLocation.endsWith("/")) {
            resourceLocation = resourceLocation + "/";
        }
        registry.addResourceHandler("/file/**").addResourceLocations(resourceLocation);
    }
