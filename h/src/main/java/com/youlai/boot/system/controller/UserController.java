...
public class UserController {
    ...
    @Operation(summary = "超级管理员用户分页列表")
    @GetMapping("/page/admin")
    @Log(value = "超级管理员用户分页列表", module = LogModuleEnum.USER)
    public PageResult<UserPageVO> getAdminUserPage(
            @Valid UserPageQuery queryParams
    ) {
        queryParams.setRoleId(1L);
        IPage<UserPageVO> result = userService.getUserPage(queryParams);
        return PageResult.success(result);
    }

    @Operation(summary = "省级管理员用户分页列表")
    @GetMapping("/page/province")
    @Log(value = "省级管理员用户分页列表", module = LogModuleEnum.USER)
    public PageResult<UserPageVO> getProvinceUserPage(
            @Valid UserPageQuery queryParams
    ) {
        queryParams.setRoleId(2L);
        IPage<UserPageVO> result = userService.getProvinceUserPage(queryParams);
        return PageResult.success(result);
    }

    @Operation(summary = "监狱管理员用户分页列表")
    @GetMapping("/page/Prison")
    @Log(value = "监狱管理员用户分页列表", module = LogModuleEnum.USER)
    public PageResult<UserPageVO> getPrisonUserPage(
            @Valid UserPageQuery queryParams
    ) {
        queryParams.setRoleId(3L);
        IPage<UserPageVO> result = userService.getPrisonUserPage(queryParams);
        return PageResult.success(result);
    }
    ...
    @Operation(summary = "修改用户功能授权")
    @PutMapping(value = "/updateUserPermission/{userId}")
    @PreAuthorize("@ss.hasPerm('sys:user:edit')")
    @Log(value = "修改用户", module = LogModuleEnum.USER)
    @OperLog(value = "修改用户功能授权")
    public Result<Void> updateUserPermission(
            @Parameter(description = "用户ID") @PathVariable Long userId,
            @RequestBody @Valid UserForm userForm
    ) {
        boolean result = userService.updateUserPermission(userId, userForm);
        return Result.judge(result);
    }

    @Operation(summary = "修改用户管理区域")
    @PutMapping(value = "/updateUserArea/{userId}")
    @PreAuthorize("@ss.hasPerm('sys:user:edit')")
    @Log(value = "修改用户", module = LogModuleEnum.USER)
    public Result<Void> updateUserArea(
            @Parameter(description = "用户ID") @PathVariable Long userId,
            @RequestBody @Valid UserForm userForm
    ) {
        boolean result = userService.updateUserArea(userId, userForm);
        return Result.judge(result);
    }
